...
namespace Octgn.Networking
{
    ...
    public sealed class Client
    {
        ...
        internal void Disconnected()
        {
            ...
            if (K.C.Get<Dispatcher>() != null)
                K.C.Get<Dispatcher>().Invoke(new Action<string>(K.C.Get<GameplayTrace>().TraceWarning),
                                          "You have been disconnected from server.");
            else
                K.C.Get<GameplayTrace>().TraceWarning("You have been disconnected from server.");
        }
        ...
        private void BinaryReceive(int count)
        {
            ...
            while (_packetPos > 4)
            {
                ...
                // Invoke the handler
                K.C.Get<Dispatcher>().BeginInvoke(DispatcherPriority.Normal, _binHandler, data);

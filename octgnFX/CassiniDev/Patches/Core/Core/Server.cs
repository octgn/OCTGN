    ...
    public class Server : MarshalByRefObject, IDisposable
    {
        ...
        public void OnRequestEnd(Connection conn, string userName )
        {
            try
            {
                ...
                LogInfo reqLog = conn.RequestLog.Clone();
                reqLog.Identity = userName;
                LogInfo respLog = conn.ResponseLog.Clone();
                respLog.Identity = userName;
                OnRequestComplete(conn.Id, reqLog, respLog);
            }
            catch
            {

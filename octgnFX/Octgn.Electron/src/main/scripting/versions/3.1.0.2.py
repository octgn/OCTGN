# OCTGN Python API v3.1.0.2 — Ported from WPF IronPython to Skulpt
# This file defines the game-facing Python API classes and functions.
# It delegates all work to the injected _api global (a JS proxy).
...
def webRead(url, timeout=0):
    ...
    return (apiResult[0], apiResult[1])
...
def askChoice(question, choices=[], colors=[], customButtons=[]):
    if len(colors) != len(choices):
        ...
        for count in choices:
            colors.append('None')
    apiResult = _api.AskChoice(question, choices, colors, customButtons)
    return apiResult
...
def askMarker():
    ...
    if apiResult == None:
        return (None, 0)
    return ((apiResult[0], apiResult[1]), apiResult[2])

def askCard(properties={}, operator=None, title="Choose card"):
    apiResult = _api.AskCard(properties, operator, title)
    if apiResult == None:
        return (None, 0)
    return (apiResult[0], apiResult[1])

def queryCard(properties={}, exact=False):
    apiResult = _api.QueryCard(properties, exact)
    if apiResult == None:
        return []
...
def focus(cards=None):
    if cards == None:
        ...
        _api.Focus([c._id for c in cards])
...
class Markers(object):
    ...
    def __getitem__(self, key):
        return _api.MarkerGetCount(self._cardId, key[1], key[0])

    def __setitem__(self, key, value):
        _api.MarkerSetCount(self._cardId, value, key[1], key[0])

    def __delitem__(self, key):
        _api.MarkerSetCount(self._cardId, 0, key[1], key[0])

    def __len__(self):
        return len(_api.CardGetMarkers(self._cardId))

    def __iter__(self):
        markers = _api.CardGetMarkers(self._cardId)
        for m in markers:
            yield (m[0], m[1])

    def __contains__(self, key):
        return _api.MarkerGetCount(self._cardId, key[1], key[0]) > 0
...
class CardProperties(object):
    ...
    def __len__(self):
        return len(cardProperties)

    def __iter__(self):
        return iter(cardProperties)
...
class Card(object):
    ...
    def __getattr__(self, name):
        lname = name.lower()
        if lname == "name":
            return _api.CardName(self._id)
        if lname in cardProperties:
            return self.properties[name]
        raise AttributeError("Card has no attribute '{}'".format(name))

    def __setattr__(self, name, val):
        lname = name.lower()
        if lname == "id" or lname == "name":
            return
        if lname in cardProperties:
    ...
    @property
    def alternate(self):
        return _api.CardAlternate(self._id)
    ...
    @property
    def alternates(self):
        return _api.CardAlternates(self._id)

    def alternateProperty(self, alt, prop):
        return _api.CardProperty(self._id, prop)

    def defaultProperty(self, alt, prop):
        return _api.CardProperty(self._id, prop)

    @property
    def model(self):
        return _api.CardModel(self._id)
    ...
    @property
    def set(self):
        return _api.CardSet(self._id)

    @property
    def setId(self):
        return _api.CardSetId(self._id)

    @property
    def properties(self):
        return self._props

    def hasProperty(self, prop, alt=None):
        return _api.CardHasProperty(self._id, prop, alt)

    @property
    def owner(self):
        return Player(_api.CardOwner(self._id))
    ...
    def controller(self, player):
        _api.SetController(self._id, player._id)

    @property
    def group(self):
        gid = _api.CardGroup(self._id)
        ctor = _api.GroupCtor(gid)
        # Use the scope's eval to instantiate
        if ctor == "Table()":
            return table
        # Parse Pile(id, name, Player(pid))
        return _evalGroupCtor(ctor)

    @property
    def isFaceUp(self):
        return _api.CardGetFaceUp(self._id)

    @isFaceUp.setter
    def isFaceUp(self, value):
        _api.CardSetFaceUp(self._id, value)

    @property
    def orientation(self):
        return _api.CardGetOrientation(self._id)

    @orientation.setter
    def orientation(self, value):
        _api.CardSetOrientation(self._id, value)

    @property
    def highlight(self):
        return _api.CardGetHighlight(self._id)

    @highlight.setter
    def highlight(self, value):
        _api.CardSetHighlight(self._id, value)

    @property
    def filter(self):
        return _api.CardGetFilter(self._id)

    @filter.setter
    def filter(self, value):
        _api.CardSetFilter(self._id, value)

    @property
    def position(self):
        return _api.CardPosition(self._id)

    @property
    def height(self):
        return _api.RealHeight(self._id)

    @property
    def width(self):
        return _api.RealWidth(self._id)

    @property
    def size(self):
        return _api.CardSize(self._id)

    @property
    def markers(self):
        if self._markers == None:
            self._markers = Markers(self)
    ...
    @property
    def index(self):
        return _api.CardGetIndex(self._id)
    ...
    @property
    def isSelected(self):
        return _api.CardIsSelected(self._id)

    def select(self, selection=True):
        _api.CardSelect(self._id, selection)

    def peek(self):
        _api.CardPeek(self._id)

    @property
    def peekers(self):
        return [Player(id) for id in _api.CardPeekers(self._id)]

    def target(self, active=True):
        _api.CardTarget(self._id, active)

    def arrow(self, targetCard, active=True):
        _api.CardTargetArrow(self._id, targetCard._id, active)
...
class Group(NamedObject):
    ...
    def __len__(self):
        return _api.GroupCount(self._id)
    ...
    @property
    def visibility(self):
        return _api.GroupGetVisibility(self._id)

    @visibility.setter
    def visibility(self, value):
        _api.GroupSetVisibility(self._id, value)

    @property
    def viewers(self):
        return [Player(id) for id in _api.GroupViewers(self._id)]

    def addViewer(self, player):
        _api.GroupAddViewer(self._id, player._id)

    def removeViewer(self, player):
        _api.GroupRemoveViewer(self._id, player._id)
...
class Table(Group):
    ...
    @property
    def boards(self):
        return _api.GetBoardList()

    _twoSided = None

    @staticmethod
    def isTwoSided():
        if Table._twoSided == None:
            Table._twoSided = _api.IsTwoSided()
...
class Pile(Group):
    ...
    def shuffle(self):
        _api.GroupShuffle(self._id)

    @property
    def collapsed(self):
        return _api.GroupGetCollapsed(self._id)

    @collapsed.setter
    def collapsed(self, value):
        _api.GroupSetCollapsed(self._id, value)

    @property
    def viewState(self):
        return _api.PileGetViewState(self._id)

    @viewState.setter
    def viewState(self, value):
        _api.PileSetViewState(self._id, value)

    @property
    def protectionState(self):
        return _api.PileGetProtectionState(self._id)

    @protectionState.setter
    def protectionState(self, value):
        _api.PileSetProtectionState(self._id, value)

    def lookAt(self, value, istop=True):
        _api.GroupLookAt(self._id, value, istop)
...
class Counter(NamedObject):
    ...
    @property
    def value(self):
        return _api.CounterGet(self._id)

    @value.setter
    def value(self, value):
        _api.CounterSet(self._id, value)
...
class idict(object):
    ...
    def __iter__(self):
        for key, value in self._dict.values():
            yield key
...
class Player(object):
    def __init__(self, id):
        self._id = id
        counters_data = _api.PlayerCounters(id)
        self._counters = idict(
            (pair[1], Counter(pair[0], pair[1], self))
            for pair in counters_data
        )
        piles_data = _api.PlayerPiles(id)
        self._piles = idict(
            (pair[1], Pile(pair[0], pair[1], self))
            for pair in piles_data
        )
    ...
    def __getattr__(self, name):
        if name in self._piles:
            return self._piles[name]
        if name in self._counters:
            return self._counters[name].value
        raise AttributeError("Player has no attribute '{}'".format(name))

    def __setattr__(self, name, value):
        if not name.startswith("_"):
            if name in self._piles:
                raise AttributeError("You can't overwrite a Player's pile.")
            if name in self._counters:
    ...
    def setActive(self, force=False):
        _api.NextTurn(self._id, force)

    @property
    def isSubscriber(self):
        return _api.IsSubscriber(self._id)

    @property
    def name(self):
        return _api.PlayerName(self._id)
    ...
    @property
    def color(self):
        return _api.PlayerColor(self._id)

    @color.setter
    def color(self, colorHex):
        _api.SetPlayerColor(self._id, colorHex)

    @property
    def isInverted(self):
        return _api.PlayerHasInvertedTable(self._id)
...
def _evalGroupCtor(ctor):
    """Parse a GroupCtor string like Pile(id, 'name', Player(pid)) into a Pile object."""
    if ctor.startswith("Table"):
        return table
    if ctor.startswith("Pile("):
        inner = ctor[5:-1]
        # Parse: id, 'name', Player(pid)
        parts = inner.split(", ", 2)
        gid = int(parts[0])
        gname = parts[1].strip("'")
        # Extract player id from Player(N)
        pid_str = parts[2]
        pid = int(pid_str[7:-1])  # "Player(N)" -> N
        return Pile(gid, gname, Player(pid))
    return None

...
#pragma warning restore 649
    ...
    private CollectionContainer groupMenu = new CollectionContainer();
    private CollectionContainer cardMenu = new CollectionContainer();

    protected Card contextCard;   // don't make this static. Timing with Keep/ReleaseControl and ContextMenu can create bug when field is shared amongst groups.
    protected Group contextGroup; // obviously this is equal to group. But as the control gets unload / reloaded, group gets null/non null in .NET 4.
    ...
    protected virtual void OnCardOver(object sender, CardsEventArgs e)
    {
      ...
      switch (group.Visibility)
      {
        case GroupVisibility.Nobody:
          e.FaceUp = false; break;
        case GroupVisibility.Everybody:
          e.FaceUp = true; break;
        case GroupVisibility.Owner:
          e.FaceUp = group.Owner == Player.LocalPlayer; break;
        ...
        case GroupVisibility.Custom:
          e.FaceUp = group.viewers.Contains(Player.LocalPlayer); break;
        default:
    ...
    public void ExecuteDefaultAction(Card card)
    {
      if (defaultCardAction != null)
      {
        ...
        group.KeepControl(); card.KeepControl();
        ...
        group.ReleaseControl(); card.ReleaseControl();
      }
      else
    ...
    internal int GetTurnAnimationDelay()
    {
      ...
      var delay = turnAnimationDelay - (currentTimestamp - turnAnimationTimestamp);
      if (delay < 0) delay = 0;
    ...
    internal virtual void ShowContextMenu(Card card, bool showGroupActions = true)
    {
      ...
      contextGroup = group;
      ContextMenu = new ContextMenu() { ItemsSource = menuItems, Tag = card };  // card has to captured somehow, otherwise it may be overwritten before released in the OnClosed handler, e.g. when rightclicking on a card, then directly right-clicking on another card.
      ContextMenu.Opened += (sender, args) => { contextGroup.KeepControl(); var c = ((ContextMenu)sender).Tag as Card; if (c != null) c.KeepControl(); };
      ContextMenu.Closed += (sender, args) => { contextGroup.ReleaseControl(); var c = ((ContextMenu)sender).Tag as Card; if (c != null) c.ReleaseControl(); };
      ...
      if (group.CanManipulate())
      {
        if (card != null)
        {
          ...
          else
          {
            ...
            item.SetResourceReference(Control.StyleProperty, "MenuHeader");
            menuItems.Add(item);
      ...
      ContextMenu.IsOpen = false;     // Required to trigger the ReleaseControl calls if the ContextMenu was already open
    ...
    private MenuItem CreateVisibilityItem()
    {
      ...
      {
        ((MenuItem)item.Items[0]).IsChecked = group.Visibility == GroupVisibility.Nobody;
        ((MenuItem)item.Items[1]).IsChecked = group.Visibility == GroupVisibility.Everybody;
        ...
        foreach (Player p in Player.AllExceptGlobal)
        {
          playerItem = new MenuItem()
          {
            ...
            Tag = p,
            IsCheckable = true,
            IsChecked = group.Visibility == GroupVisibility.Custom && group.viewers.Contains(p),
          };
    ...
    private MenuItem CreateGroupPassToItem()
    {
      ...
      passToItem.SubmenuOpened += delegate
      {
        ...
        foreach (Player player in Player.AllExceptGlobal)
          ...
          {
            var playerItem = new MenuItem() { Header = player.Name, Tag = player };
            playerItem.Click += delegate(object sender, RoutedEventArgs e)
            {
              Player p = (Player)((MenuItem)sender).Tag;
      ...
      passToItem.SubmenuOpened += delegate
      {
        ...
        foreach (Player player in Player.AllExceptGlobal)
          if (player != Player.LocalPlayer)
          {
            ...
            {
              Player p = (Player)((MenuItem)sender).Tag;
              Selection.Do(c => c.PassControlTo(p), contextCard);
            };
            passToItem.Items.Add(playerItem);
          }
        if (!passToItem.HasItems)
    ...
    private MenuItem CreateGroupMenuItem(BaseActionDef baseAction)
    {
      ...
      if (group != null)
      {
        foreach (var subItem in group.Children.Select(x => CreateGroupMenuItem(x)))

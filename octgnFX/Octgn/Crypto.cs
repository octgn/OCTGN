...
namespace Octgn
{
	static class Crypto
	{
		...
		{
			BigInteger k = new BigInteger(new uint[] { PositiveRandom(), PositiveRandom() });
			BigInteger c1 = b.ModPow(k, m), c2 = new BigInteger(pkey).ModPow(k, m);
			BigInteger encrypted = (c2 * new BigInteger(data)) % m;
			return new ulong[] { (ulong)c1.LongValue(), (ulong)encrypted.LongValue() };
		}
		...
		public static ulong[] Encrypt(Guid data, ulong pkey)
		{
			...
			return new ulong[] { (ulong)c1.LongValue(), 
		...
		public static Guid DecryptGuid(ulong[] data)
		{
			...
			for (int i = 1; i < 5; i++)
			{
				BigInteger c2 = new BigInteger(data[i]);

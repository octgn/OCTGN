...
namespace Octgn.Lobby.Server
{
    public static class GameBot
    {
        ...
        private static ThreadSafeList<Jid> UserList { get; set; }
        ...
        public static void RemakeXmpp()
        {
            ...
            Xmpp.OnXmppConnectionStateChanged += XmppOnOnXmppConnectionStateChanged;
            UserList = new ThreadSafeList<Jid>();
            Xmpp.Open();
        }
        ...
        static void Xmpp_OnAuthError(object sender, Element e)
        {
        ...
        private static void XmppOnOnMessage(object sender , Message msg)
        {
            switch(msg.Type)
            {
                case MessageType.normal:
                    ...
                    {
                        var data = msg.Body.Split(new string[]{",:,"},StringSplitOptions.RemoveEmptyEntries);
                        ...
                        if(Guid.TryParse(data[0] , out guid) && Version.TryParse(data[1] , out ver))
                        {
                            ...
                            if (port == -1) return;
                            var m = new Message(msg.From , msg.To , MessageType.normal , port.ToString(CultureInfo.InvariantCulture) , "gameready");
                            m.GenerateId();
                            Xmpp.Send(m);
